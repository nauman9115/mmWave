...
MmWaveUePhy::DoInitialize (void)
{
	...
	NS_LOG_INFO("SNN::mmwave-ue-phy.cc In DoInitialize::");
...
MmWaveUePhy::RegisterToEnb (uint16_t cellId, Ptr<MmWavePhyMacCommon> config)
{
	...
	NS_LOG_INFO("SNN::mmwave-ue-phy.cc In RegisterToEnb");
...
MmWaveUePhy::StartSlot ()
{
	...
	NS_LOG_INFO ("START_SLOT::: MmWave UE " << m_rnti << " frame " << m_frameNum << " subframe " << (uint16_t) m_sfNum << " slot " << (uint16_t) m_slotNum);
	...
	if (m_slotNum == 0)  // reserved DL control
	{
		...
		NS_LOG_DEBUG ("DL CONTROL::::::UE" << m_rnti << " imsi" << m_imsi << " RXing DL CTRL frame " << m_frameNum << " subframe " << (unsigned)m_sfNum << " symbols "
	...
	else if (m_slotNum == m_currSfAllocInfo.m_slotAllocInfo.size()-1) // reserved UL control
	{
		...
		NS_LOG_DEBUG ("UL CONTROL::::UE" << m_rnti << " imsi" << m_imsi << " TXing UL CTRL frame " << m_frameNum << " subframe " << (unsigned)m_sfNum << " symbols "
		              << (unsigned)currSlot.m_dci.m_symStart << "-" << (unsigned)(currSlot.m_dci.m_symStart+currSlot.m_dci.m_numSym-1) <<
	...
	else if (currSlot.m_dci.m_format == DciInfoElementTdma::DL_dci)  // scheduled DL data slot
	{
		...
		NS_LOG_DEBUG ("DL DATA:::::::UE" << m_rnti << " imsi" << m_imsi << " RXing DL DATA frame " << m_frameNum << " subframe " << (unsigned)m_sfNum << " symbols "
		              << (unsigned)currSlot.m_dci.m_symStart << "-" << (unsigned)(currSlot.m_dci.m_symStart+currSlot.m_dci.m_numSym-1) <<
		              "\t start " << Simulator::Now() << " end " << (Simulator::Now()+slotPeriod));
	}
	else if (currSlot.m_dci.m_format == DciInfoElementTdma::UL_dci) // scheduled UL data slot
	{
		...
		NS_LOG_DEBUG ("UL DATA:::::UE" << m_rnti << " imsi" << m_imsi << " TXing UL DATA frame " << m_frameNum << " subframe " << (unsigned)m_sfNum << " symbols "
		              << (unsigned)currSlot.m_dci.m_symStart << "-" << (unsigned)(currSlot.m_dci.m_symStart+currSlot.m_dci.m_numSym-1)
		              << "\t start " << Simulator::Now() << " end " << (Simulator::Now()+slotPeriod));
		if(pktBurst != 0)
		{
	...
	NS_LOG_INFO ("START_SLOT:::::m_slotNum " << (uint16_t)m_slotNum);
...
MmWaveUePhy::EndSlot ()
{
	...
	NS_LOG_INFO ("END_SLOT:::::m_slotNum " << (uint16_t)m_slotNum);
